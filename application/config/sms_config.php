<?php
// set path zone
$config['template_path'] = 'assets/';	
$config['uploads_dir'] = 'uploads/';
$config['admin'] = '/admin';
// end set path zone

// set data for start system  zone
$config['title_system'] = 'MED';
$config['title_sub_system'] = 'SMS';
$config['title_system_th'] = 'ระบบบริหารแผนยุทธศาสตร์';
$config['title_web'] = 'ระบบบริหารแผนยุทธศาสตร์ คณะแพทยศาสตร์มหาวิทยาลัยบูรพา';
$config['org_name'] = '';
$config['title_header'] = 'AadminLTE for System';
// end set data for start system  zone

// set config type
$config['panel_header_color'] = "primary"; //หัว box
$config['btn_add_color'] = "btn btn-primary"; //ปุ่มเพิ่ม
$config['btn_del_color'] = "btn btn-danger"; //ปุ่มลบ
$config['btn_edit_color'] = "btn btn-warning"; //ปุ่มแก้ไข
$config['btn_set_kpi_color'] = "btn btn-info"; //ปุ่มแก้ไข
$config['btn_success'] = "btn btn-success"; //ปุ่มยืนยัน
$config['sms_icon_add'] = "fa fa-plus"; //icon add
$config['sms_icon_edit'] = "glyphicon glyphicon-pencil"; //icon edit
$config['sms_icon_del'] = "fa fa-trash"; //icon delete
$config['sms_icon_set_kpi'] = "fa fa-cog"; //icon delete
// end set config color

// set config color
$config['table_header'] = "#66b3ff"; //สีหัวตาราง
$config['table_tr_show_info'] = "#B0B0B0"; //สี col 1 แสดงข้อมูล
$config['red_color'] = "#dd4b39"; //รหัสสี * 
// end set config color

// set text
$config['txt_cancel'] = "ยกเลิก";
$config['txt_save'] = "บันทึก";

// end set text

?>